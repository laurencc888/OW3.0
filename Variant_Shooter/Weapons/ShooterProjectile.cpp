...
AShooterProjectile::AShooterProjectile()
{
	...
	// when projectile is spawned on the client, it will also spawn on the server
	SetReplicates(true);
}
...
void AShooterProjectile::ProcessHit(AActor* HitActor, UPrimitiveComponent* HitComp, const FVector& HitLocation, const FVector& HitDirection)
{
	...
	if (ACharacter* HitCharacter = Cast<ACharacter>(HitActor))
	{
		...
		{
			AShooterCharacter* HitChar = Cast<AShooterCharacter>(HitCharacter);
			AShooterCharacter* Character = Cast<AShooterCharacter>(GetInstigator());

			if (!(HitChar && Character && (HitChar->Team == Character->Team)))
			{
